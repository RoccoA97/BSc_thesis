...
\footnotetext{
The value of the $\chi^2$ reported in these plots and in the ones in the following Chapter is the result of a $\chi^2$ test applied to the $N$ bins of the histogram in order to verify the compatibility of $t$ distribution with the expected one. Denoting with $O_i$ and $E_i$ respectively the observed and the expected values for the $i^\text{th}$ bin, it is computed through:
\begin{equation}
